...
let JwtAuthGuard = class JwtAuthGuard extends (0, passport_1.AuthGuard)('jwt') {
    ...
    async canActivate(context) {
        ...
        if (!userFromDb || !userFromDb.token) {
            throw new common_1.UnauthorizedException('No valid session found');
        }
        if (userFromDb.token !== token) {
            throw new common_1.UnauthorizedException('You have been logged out because of login from another device');
        }
        return true;
    }
};
